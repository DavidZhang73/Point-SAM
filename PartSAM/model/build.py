...
def fn(module: nn.Module):
    for n, m in module.named_children():
        if isinstance(m, nn.LayerNorm):
            module.register_module(n, FusedLayerNorm(m.normalized_shape, m.eps, m.elementwise_affine))


def build_sam(cfg):
    point_transformer = timm.create_model(cfg.encoder_model_name, drop_path_rate=cfg.drop_path_rate)
    ...
    prompt_encoder = PromptEncoderNN(cfg.prompt_embed_dim, cfg.encoder_num_group, cfg.encoder_group_size)
