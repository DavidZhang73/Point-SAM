...
def format_time_brief(seconds: int | float) -> str:
    """Convert the seconds to human readable string with days, hours, minutes and seconds."""
    s = int(np.rint(seconds))
...
class TimingsMonitor:
    ...
    def record_timing(self, name, value):
        if name not in self.all_timings_dict:
            self.add_new_timing(name)
    ...
    def _record_cuda_event_start(self, name):
        if name in self.cuda_event_timers:
            self.cuda_event_timers[name].start.record(torch.cuda.current_stream(self.device))

    def _record_cuda_event_end(self, name):
        if name in self.cuda_event_timers:
            self.cuda_event_timers[name].end.record(torch.cuda.current_stream(self.device))

    @contextmanager
    def cuda_timing_scope(self, name, profile=True):
        if name not in self.all_timings_dict:
            self.add_new_cuda_timing(name)
    ...
    def record_all_cuda_timings(self):
        ...
        for k, events in self.cuda_event_timers.items():
            with torch.no_grad():
                ...
                # Convert to seconds
                time_elapsed = events.start.elapsed_time(events.end) / 1000.0
...
def download_from_s3(file_path, target_path, cfg):
    ...
    logger.info(f"finish download from ! s3://{bucket_name}/{file_key} to {target_path} %.1f sec" % (time.time() - tic))
...
def write_ckpt_to_s3(cfg, all_model_dict, ckpt_name):
    ...
    config_dict = json.load(open(cfg.checkpoint.write_s3_config))
...
def printarr(*arrs, float_width=6, **kwargs):
    ...
    def minmaxmean_str(a):
        ...
        try:
            ...
            pass
        try:
            median_str = format_float(a.median())
        except:
            try:
                median_str = format_float(np.median(np.array(a)))
            except:
                median_str = "N/A"
