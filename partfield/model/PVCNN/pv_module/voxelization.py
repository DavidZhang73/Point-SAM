...
def my_voxelization(features, coords, resolution):
    ...
    features = torch.cat(
        [features, torch.ones(features.shape[0], 1, features.shape[2], device=features.device, dtype=features.dtype)],
        dim=1,
    )
    ...
    def forward(self, features, coords):
        with torch.no_grad():
            ...
            if self.normalize:
                norm_coords = (
                    norm_coords
                    / (norm_coords.norm(dim=1, keepdim=True).max(dim=2, keepdim=True).values * 2.0 + self.eps)
                    + 0.5
                )
            else:
                if self.scale_pvcnn:
