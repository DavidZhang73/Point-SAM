...
from .pv_module import PVConv, SharedMLP
...
class PVCNNEncoder(nn.Module):
    def __init__(self, pvcnn_feat_dim, device="cuda", in_channels=3, use_2d_feat=False, is_My=False):
        ...
        layers, channels_point, concat_channels_point = create_pointnet_components(
            blocks=self.blocks,
            in_channels=in_channels + self.append_channel,
            with_se=False,
            normalize=False,
            width_multiplier=1,
            voxel_resolution_multiplier=1,
    ...
    def forward(self, input_pc, mv_feat=None, pc2pc_idx=None, add_features=None):
        ...
        zero_padding = torch.zeros(
            features.shape[0], self.append_channel, features.shape[-1], device=features.device, dtype=torch.float
        )
        ...
        for i in range(len(self.encoder)):
            ...
            if i == 0 and self.is_My:
                features_add1, _, voxel_feature_add1 = self.add_layer[0](
                    (torch.cat([add_features.permute(0, 2, 1), zero_padding], dim=1), coords)
                )
