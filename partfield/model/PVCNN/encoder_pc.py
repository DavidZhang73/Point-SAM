    ...
    def __init__(
        ...
        if point_encoder_type == "pvcnn":
            self.pc_encoder = PVCNNEncoder(
                point_encoder_out_dim, device=self.device, in_channels=in_channels, use_2d_feat=use_2d_feat, is_My=is_My
        ...
        self.unet_encoder = setup_unet(
            output_channels=point_encoder_out_dim, input_channels=point_encoder_out_dim, unet_cfg=unet_cfg
        )
        ...
        if self.point_encoder_type == "pvcnn":
            if mv_feat is not None:
                ...
                pc_feat, points_feat = self.pc_encoder(
                    point_cloud, add_features=add_features
