...
from .dataloader import Correspondence_Demo_Dataset, Demo_Dataset, Demo_Remesh_Dataset
    ...
    def __init__(self, cfg):
        ...
        if self.use_pvcnn:
            self.pvcnn = TriPlanePC2Encoder(
                cfg.pvcnn, device="cuda", shape_min=-1, shape_length=2, use_2d_feat=self.use_2d_feat
            )  # .cuda()
        ...
        if self.cfg.is_pc:
            ...
            if use_cuda_version:
                ...
                def sample_points(vertices, faces, n_point_per_face):
                    ...
                    n_f = faces.shape[0]
                    u = torch.sqrt(torch.rand((n_f, n_point_per_face, 1), device=vertices.device, dtype=vertices.dtype))
                    v = torch.rand((n_f, n_point_per_face, 1), device=vertices.device, dtype=vertices.dtype)
                    w0 = 1 - u
                    w1 = u * (1 - v)
                    w2 = u * v
                    ...
                    points = (
                        w0 * face_v_0.unsqueeze(dim=1) + w1 * face_v_1.unsqueeze(dim=1) + w2 * face_v_2.unsqueeze(dim=1)
                    )
                    return points
